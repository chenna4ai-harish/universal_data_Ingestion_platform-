...
STATUS_COLORS = {
    "SUCCESS": "✅ SUCCESS",
    "SUCCESS_WITH_EXCEPTIONS": "⚠️ SUCCESS WITH EXCEPTIONS",
    "BLOCKED": "🚫 BLOCKED",
    "FAILED": "❌ FAILED",
}
...
def run_ingestion(
    ...
    # Parse user column overrides from text box
    user_overrides = {}
    ...
    tbl1_label = tbl_names[0] if tbl_names else "Table 1"
    tbl2_label = tbl_names[1] if len(tbl_names) > 1 else "Table 2"
...
def build_ui() -> gr.Blocks:
    with gr.Blocks(
        ...
        with gr.Tabs():
            ...
            # TAB 5 — Downloads
            # ----------------------------------------------------------------
            with gr.Tab("⬇️ Download Outputs"):
